...
| An empty body is a **trap**, not a blank field | Descriptions are not persisted in the committed ledger and `state.db` rebuilds from it, so any posting that has round-tripped through CI comes back with no body. Nothing downstream noticed, because `compress_jd("")` returns `""` and every consumer treated that as "this posting asked for nothing". It bit twice. In the email, `hydrate_all()` was handed database-loaded jobs whose `needs_hydration` flag only the fetchers ever set, so the top-up printed "re-hydrated 0/5" and fetched nothing — every backlog posting reached the inbox claiming "ATS keywords: none detected". In `pipeline.tailor`, the command the email prints under every posting, the same empty body went to the model as a tailoring brief with no job description in it, at full price, producing a resume tailored to nothing. Both now top up over free HTTP first; `tailor` refuses with exit 4 rather than bill for a brief it could not fill. |
...
| `budget.max_usd_per_run: 0.15` and `limits.max_new_scores_per_run: 17` move **together** | Raised from $0.10 on 2026-08-21 to buy capacity. They are coupled and nothing enforced it before `test_the_budget_ceiling_covers_a_full_scoring_run`: the ceiling aborts *before* the call that would breach it, so a cap that outgrows its budget does not fail loudly — it scores part of the pool and the email quietly draws on a thinner backlog. **Size the cap from `out/usage.jsonl`, never from list pricing.** Three real runs cost $0.00330–$0.00345 per posting scored (28/$0.0965, 28/$0.0926, 20/$0.0659), so 17 per track spends ~$0.119, 79% of the ceiling. A first attempt at this used a *derived* $0.00158 — it assumed every batch call carried a full `score_batch_size` of 8, where real runs average nearer 7 — and set the cap at 30, which would have cost ~$0.21 and aborted mid-run. Note the old $0.10 ceiling was already nearly binding: the 2026-08-19 run cost $0.0965. |
| Scoring capacity is the binding constraint, not the fit floor | A posting cannot be sent until it is scored. One fetch brought in 60 software and 50 hardware postings, and the ledger holds 223 unscored inside seven days; at 17 per track most are never scored, and because `db.unscored()` orders freshest-first they can never catch up — the next morning's arrivals outrank them permanently. That is a deliberate, budgeted loss, not a bug, and it is why freshest-first ordering matters so much: the postings that get skipped are always the least fresh ones. If the email is thin, check this and the fetch before touching `min_fit`. |
...
6. **Seasonality.** Inflow was measured in August, at peak new-grad hiring, and
   ...
   Most postings are never scored on a heavy day: 17 per track against 223
   unscored inside seven days. That is a money dial rather than a bug — the
   two settings that move it are in the State and cost table, and they must
   move together.
