...
def _job_card(job: Job, resume: dict, jd_max_chars: int) -> str:
...
def _track_section(title: str, sel: Selection, resume: dict, jd_max_chars: int) -> str:
    ...
    for job in sel.jobs:
        parts.append(_job_card(job, resume, jd_max_chars))
    return "".join(parts)


def _plain_text(software: Selection, hardware: Selection) -> str:
    lines: list[str] = []
...
def render_email(
    ...
    *,
    ...
    return RenderedEmail(
        subject=subject,
        html=body,
        text=_plain_text(software, hardware),
